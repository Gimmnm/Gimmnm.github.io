---
title: "FlowBench: 2-D thermally coupled lid-driven cavity at constant Reynolds number"
...
summary: "At fixed inertial/viscous conditions, buoyancy strength and complex geometry are varied to produce steady velocity, pressure, and temperature fields."
description: "At fixed inertial/viscous conditions, buoyancy strength and complex geometry are varied to produce steady velocity, pressure, and temperature fields."
...
FlowBench calls the coupled system **Navier–Stokes + Heat Transfer (NSHT)** and specifies Boussinesq coupling. A standard nondimensional strong form consistent with the paper is
...
The nondimensional parameters satisfy
...
Thus the simulations effectively fix $\mathrm{Pr}=0.7$. The paper focuses on the SBM weak form and does not fix the sign convention of the buoyancy term in the dataset section; that sign depends on gravity and reference-temperature conventions.
...
Velocity conditions are identical to the pure-NS LDC: the top wall has $(u,v)=(1,0)$ and all other outer walls and the object are stationary no-slip boundaries.

Temperature conditions are

$$
\theta=1\quad\text{on the bottom wall},
$$

$$
\theta=0\quad\text{on the top wall},
$$

$$
\frac{\partial\theta}{\partial n}=0
\quad\text{on the left and right walls},
$$

$$
\theta=0\quad\text{on the object surface}.
$$
...
| Official DataPrep sample count | **2990** |
| Combined NSHT count in the paper | **5990** across constant-Re and variable-Re |
| Reynolds number | fixed; the paper's fixed-Re examples use $\mathrm{Re}=100$ |
| Buoyancy parameter | varying $\mathrm{Gr}$, equivalently varying $\mathrm{Ri}$ |
| Overall paper Grashof range | $\mathrm{Gr}\in[10,10^7]$ |
| Resolutions | $128^2$, $256^2$, $512^2$ |
| Trajectory length | one final steady snapshot |
| Current hosted size | approximately **34.5 GB** |
| Repository path | `LDC_NSHT_2D_constant-Re/` |

### Core tensors

The split-specific DataPrep convention is
...
because the fixed Reynolds number carries no sample-dependent information. The physical output fields are
...
The preparation README may append a `C*` channel containing packaged $C_D,C_L,\mathrm{Nu}$ summaries. It is not a fifth local PDE state.

## Adjustable, varied, and fixed quantities

| Quantity | Treatment |
|---|---|
| geometry | varied across G1/G2/G3 |
| Reynolds number | fixed; paper examples use 100 |
| Grashof number | varied |
| Richardson number | varied through $\mathrm{Ri}=\mathrm{Gr}/\mathrm{Re}^2$ |
| Péclet number | fixed with the fixed Reynolds number |
| Prandtl number | fixed at 0.7 |
| velocity BCs | fixed |
| thermal BCs | fixed |
| object position | fixed |
| independent density/viscosity/diffusivity scan | absent |
| time | steady solution only |

## Physical regimes

The paper uses approximately

- $\mathrm{Ri}\approx0.1$ for forced-convection-dominated cases;
- intermediate values for mixed convection;
- $\mathrm{Ri}\approx10$ for strong natural/free convection.
...
- geometry controls both hydrodynamic separation and thermal boundary layers;
- the collection spans forced, mixed, and natural convection;
- temperature errors feed back into velocity and pressure through buoyancy;
- $C_D,C_L,$ and $\mathrm{Nu}$ are sensitive to near-wall gradients;
- the subset supports coupled-operator learning, geometry OOD, and multiphysics residual evaluation.

## Release and file caveats

1. The paper combines both NSHT splits into 5990 samples; the repository separates 2990 constant-Re and 3000 variable-Re samples.
2. The paper's unified table uses `[Re, Gr, g, s]`, while the constant-Re preparation script uses `[Gr, g, s]`.
...
Replace `LDC_NSHT_2D_constant-Re/128x128/*` with the path shown in the code block. For the larger subsets, download one resolution, geometry family, or geometry instance first rather than the full repository.
