---
title: "CE-RP: Four-Quadrant Random Riemann Problems"
...
summary: "Random four-quadrant Riemann problems for the 2-D compressible Euler equations, producing shocks, contact discontinuities, and rarefactions."
description: "Random four-quadrant Riemann problems for the 2-D compressible Euler equations, producing shocks, contact discontinuities, and rarefactions."

---

# CE-RP: Four-Quadrant Random Riemann Problems

> **One-line description:** Random four-quadrant Riemann problems for the 2-D compressible Euler equations, producing shocks, contact discontinuities, and rarefactions.

## Longer description

This is a stochastic two-dimensional generalization of Sod-type shock-tube problems. Each quadrant has an independently sampled constant state, requiring the model to learn interacting multidirectional discontinuities.

**Dataset authors/maintainers:** The POSEIDON authors, Computational and Applied Mathematics Laboratory, ETH Zurich.  
**Code or software used to generate the data:** ALSVINN high-resolution finite-volume solver with quadratic WENO reconstruction and HLLC Riemann solver; generated at $512^2$ and downsampled to $128^2$.
...
| Subset / logical task | **CE-RP** |
| Role | Pretraining operator |
| PDE family | Compressible Euler |
| Spatial dimension | 2-D |
| Time dependent | Yes |
| Official code identifier | `fluids.compressible.Riemann` |
| Official data page | [CE-RP](https://huggingface.co/datasets/camlab-ethz/CE-RP) |
...
The unit square is split into $2\times2$ subdomains $D_{ij}$, each initialized with a constant state $(\rho_{ij},v_{x,ij},v_{y,ij},p_{ij})$.
...
| Initial condition / input | Piecewise-constant states on four quadrants. |
...
| Generation software / numerical method | ALSVINN high-resolution finite-volume solver with quadratic WENO reconstruction and HLLC Riemann solver; generated at $512^2$ and downsampled to $128^2$. |
...
|---|---|---|---|
| $\rho_{ij}$ | quadrant density; IC / data-distribution parameter | varied by quadrant and trajectory | $\mathcal U[0.1,1]$ |
| $v_{x,ij},v_{y,ij}$ | quadrant velocity; IC / data-distribution parameter | varied by quadrant and trajectory | $\mathcal U[-1,1]$ |
| $p_{ij}$ | quadrant pressure; IC / data-distribution parameter | varied by quadrant and trajectory | $\mathcal U[0.1,1]$ |
| $p_{\rm grid}$ | number of partitions per direction; IC / data-distribution parameter | fixed | $2$ |
| $\gamma$ | ratio of specific heats; PDE coefficient | fixed; not swept | $1.4$ |

**Summary:** Adjustable choices include the number of quadrants, state ranges, $\gamma$, boundary type, and domain; the release varies only the initial states across discontinuities.
...
- Expected assembled filename: `CE-RP.nc`

### Official POSEIDON model interface

- One input/output pair: `[4,128,128] → [4,128,128]`
- Channel definition: The model drops $E$, which is reconstructible from the equation of state, and uses $[\rho,v_x,v_y,p]$.
...
ALSVINN high-resolution finite-volume solver with quadratic WENO reconstruction and HLLC Riemann solver; generated at $512^2$ and downsampled to $128^2$.
...
- Interacting multidirectional shocks and contacts; the solution contains strong discontinuities.
