...
1. Raw trajectories are usually `[N,T,C,H,W]`; the all2all loader returns `[C,H,W] → [C,H,W]` plus lead time, and batching adds `B`.
2. Distinguish mathematically adjustable, generator-editable, and actually sampled parameters in the release.
3. Known source conflicts include Wave-Layer’s 21/15-frame conflict, Wave time calibration, ACE’s $\epsilon$ versus $\epsilon^2$ notation, and raw tracer channels not used by several paper tasks.
...
- `TEMPLATE_zh-CN.md`: reusable documentation template.
- `TEMPLATE_en.md`: reusable documentation template.
- `SOURCES.md`: common sources and version notes.
