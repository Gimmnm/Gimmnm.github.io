...
| 数据文件 | $(D_u,D_v,k)$ | 边界 | 每轨迹随机 | 固定 |
|---|---|---|---|---|
| `2D_diff-react_NA_NA.h5` | $(10^{-3},\,5\times10^{-3},\,5\times10^{-3})$ | Neumann | $u(0)\sim\mathcal N(0,1)$ | 域 $(-1,1)^2$；训练常用 $128\times128$，$N_t=101$ |

### 生成器可调范围

| 参数 | 可调范围 / 选项 | 发布数据是否覆盖 |
|---|---|---|
| $D_u,D_v,k$ | 均可在 YAML 中改 | 否（发布固定一组） |
| 初值噪声分布 | 可改 | 否（默认 $\mathcal N(0,1)$） |
| 分辨率 / 时间点数 | 可改（原始常更高） | 发布为下采样训练规格 |
