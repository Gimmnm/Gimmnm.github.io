...
| 数据文件 | 边界 | 每轨迹随机 | 固定（本文件） |
|---|---|---|---|
| `2D_rdb_NA_NA.h5` | Neumann | 溃坝半径 $r\sim\mathcal U(0.3,0.7)$ | 圆心居中；内 $h=2$、外 $h=1$；$g_r=1$；域 $[-2.5,2.5]^2$，$128\times128$，$N_t=101$，1,000 条 |

### 生成器可调范围

| 参数 | 可调范围 / 选项 | 发布数据是否覆盖 |
|---|---|---|
| 溃坝半径 $r$ | 抽样区间可改 | 是：轨迹内 $\mathcal U(0.3,0.7)$ |
| 内外水深、圆心、重力 $g_r$、底地形 $b$ | 均可改 | 否（发布固定） |
| 边界、域、网格、时间 | 可改 | 发布固定 |
