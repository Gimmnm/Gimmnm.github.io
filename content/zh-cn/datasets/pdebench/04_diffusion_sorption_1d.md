...
仅 1 个文件；材料参数在发布集中**固定**（`NA_NA`），但文件仍有明确数值。

| 数据文件 | $(D,\phi,\rho_s,k,n_f)$ | 边界 | 每轨迹随机 | 固定 |
|---|---|---|---|---|
| `1D_diff-sorp_NA_NA.h5` | $(5\times10^{-4},\,0.29,\,2880,\,3.5\times10^{-4},\,0.874)$ | Cauchy | $u(0,x)\sim\mathcal U(0,0.2)$ | $N_x=1024$；域 $[0,1]\times[0,500]$；常用 $N_t=101$（论文写 100；原始可至 501） |

### 生成器可调范围

| 参数 | 可调范围 / 选项 | 发布数据是否覆盖 |
|---|---|---|
| $D,\phi,\rho_s,k,n_f$ | 均可在生成配置中改 | 否（发布固定为上表一组） |
| 初始浓度分布 | 可改抽样区间/场形 | 部分：发布用 $\mathcal U(0,0.2)$ |
| 边界、域、网格、时间 | 可改 | 发布固定 |
