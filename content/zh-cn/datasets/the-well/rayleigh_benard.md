---
title: "Rayleigh–Bénard 热对流——原生网格"
...
|---|---|
| 所属集合 | The Well |
| 下载目录 | `rayleigh_benard` |
| 空间维数 | 2D |
| 坐标系 | Cartesian; Fourier in $x$, Chebyshev in wall-normal $z$ |
| 空间分辨率 | $512\times128$ |
| 每条轨迹存储帧数 | 200 |
| 轨迹数 | 1750 |
| 展开后的动态通道数 | 4 |
| 动态物理场 | 浮力 $b$、压强 $p$、速度 $(u_x,u_z)$ |
| 静态场/标量上下文 | Rayleigh 数、Prandtl 数与初始扰动标签 |
| 时间范围 | $[0,50]$ |
| 存储时间间隔 | $0.25$ |
| 空间区域 | $x\in[0,4], z\in[0,1]$ |
| 发布数据量 | approximately 342–358 GB across documentation versions |
...
该目录保存 Dedalus 原生壁法向 Chebyshev 非均匀坐标。后续 release 修正了坐标元数据；假设等间距的模型应使用坐标感知方法或 uniform 重采样版本。
...
| 资源 | URL |
|---|---|
| 官方数据页 | <https://polymathic-ai.org/the_well/datasets/rayleigh_benard/> |
| 仓库目录 | <https://github.com/PolymathicAI/the_well/tree/master/datasets/rayleigh_benard> |
| Hugging Face 数据页 | <https://huggingface.co/datasets/polymathic-ai/rayleigh_benard> |
