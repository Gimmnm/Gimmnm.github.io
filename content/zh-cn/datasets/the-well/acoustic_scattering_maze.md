...
![迷宫密度图](/the-well/acoustic_scattering_maze__mazes_density.png)

![压力演化](/the-well/acoustic_scattering_maze__pressure_normalized.gif)


> **所属数据集：** The Well
> **数据目录：** `acoustic_scattering_maze`
> **方程族：** 变系数声学
...
| 数据中实际变化 | 迷宫拓扑与声源随轨迹变化。先生成初始宽度 6–16 像素的粗迷宫，再用 nearest-neighbor 上采样到 \(256^2\)。墙体设 \(\rho=10^6\)，通道设 \(\rho=3\)。每条轨迹在通道内放置 1–6 个声压环，强度 \(\mathcal U(3,5)\)、半径 \(\mathcal U(0.01,0.04)\)，与墙体重叠部分被移除。 |
| 数据中保持固定 | 体积模量 \(K=4\)；墙体/通道密度值；网格、区域、边界和 CFL 规则。 |

## 4. 初始条件与边界条件

### 初始条件

平坦声压背景叠加 1–6 个受通道约束的声压环；初始速度为零。
...
| 时间范围 | $[0,4]$ |
| 存储时间间隔 | $4/201$ (documented storage interval) |
| 空间区域 | $[-1,1]\times[-1,1]$ |
| 发布数据量 | 311.3 GB |
...
迷宫轨迹比另外两个声学版本长一倍：在 \([0,4]\) 上保存 201 帧。论文总览把声学数据合并统计，因此没有体现这一差异。
...
| 资源 | URL |
|---|---|
| 官方数据页 | <https://polymathic-ai.org/the_well/datasets/acoustic_scattering_maze/> |
| 仓库目录 | <https://github.com/PolymathicAI/the_well/tree/master/datasets/acoustic_scattering_maze> |
| Hugging Face 数据页 | <https://huggingface.co/datasets/polymathic-ai/acoustic_scattering_maze> |
