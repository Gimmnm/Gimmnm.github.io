...
![密度演化](/the-well/turbulent_radiative_layer_2D__density_normalized.gif)


> **所属数据集：** The Well
> **数据目录：** `turbulent_radiative_layer_2D`
> **方程族：** 可压缩流体 + 辐射冷却
...
| 数据中保持固定 | 状态方程 \(\gamma=5/3\)、冷热两相及相对流动设置、二维几何、发布分辨率、101 个快照和 Athena++ 求解器配置。 |
...
|---|---|
| 所属集合 | The Well |
| 下载目录 | `turbulent_radiative_layer_2D` |
| 空间维数 | 2D |
| 坐标系 | Cartesian $(x,y)$ |
| 空间分辨率 | $128\times384$ |
| 每条轨迹存储帧数 | 101 |
...
这是跨维数迁移对中较便宜的 2D 成员。它与 3D 版共享冷却时间网格，但不是每条 3D 轨迹的字面二维切片。
...
| 资源 | URL |
|---|---|
| 官方数据页 | <https://polymathic-ai.org/the_well/datasets/turbulent_radiative_layer_2D/> |
| 仓库目录 | <https://github.com/PolymathicAI/the_well/tree/master/datasets/turbulent_radiative_layer_2D> |
| Hugging Face 数据页 | <https://huggingface.co/datasets/polymathic-ai/turbulent_radiative_layer_2D> |
