...
![温度演化](/the-well/supernova_explosion_64__temperature_normalized.gif)


> **所属数据集：** The Well
> **数据目录：** `supernova_explosion_64`
> **方程族：** 可压缩 SPH 流体 + 冷却
...
| 数据中实际变化 | 发布活动改变环境湍流随机实现以及分辨率对应的样本成员。在 \(64^3\) 分辨率下有 740 条轨迹。 论文明确给出的物理设置固定为：初始温度 \(T_0=100\) K、氢数密度 \(n_{\rm H}=44.5\,{\rm cm}^{-3}\)、太阳金属丰度 \(Z=Z_\odot\)，以及标准超新星能量尺度。 |
...
两个分辨率合计 1000 条轨迹，但分别为 740 与 260；不要假设每条低分辨率轨迹都有高分辨率对应。
...
| 资源 | URL |
|---|---|
| 官方数据页 | <https://polymathic-ai.org/the_well/datasets/supernova_explosion_64/> |
| 仓库目录 | <https://github.com/PolymathicAI/the_well/tree/master/datasets/supernova_explosion_64> |
| Hugging Face 数据页 | <https://huggingface.co/datasets/polymathic-ai/supernova_explosion_64> |
