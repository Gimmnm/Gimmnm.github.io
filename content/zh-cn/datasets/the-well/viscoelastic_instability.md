...
![构象分量 Czz](/the-well/viscoelastic_instability__czz_normalized.gif)


> **所属数据集：** The Well
> **数据目录：** `viscoelastic_instability`
> **方程族：** FENE-P 黏弹性流
...
**文档记录的求解器：** Dedalus direct numerical simulation of FENE-P channel flow
...
已弃用：处理错误引入重复/循环帧，仅为向后比较保留。旧文档中的各类别数量有时与元数据的 260 条轨迹不能完全相加。

当前仓库状态： **deprecated**。本文核对日期为 2026-07-21。实际实验必须检查 `dataset_name.yaml`、`stats.yaml`、HDF5 坐标及 release notes，以确定所用文件的精确版本。
...
| 资源 | URL |
|---|---|
| 官方数据页 | <https://polymathic-ai.org/the_well/datasets/viscoelastic_instability/> |
| 仓库目录 | <https://github.com/PolymathicAI/the_well/tree/master/datasets/viscoelastic_instability> |
| Hugging Face 数据页 | <https://huggingface.co/datasets/polymathic-ai/viscoelastic_instability> |
