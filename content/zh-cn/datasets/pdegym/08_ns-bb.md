---
title: "NS-BB：Brownian Bridge 粗糙随机初值"
...
summary: "在 Fourier 空间生成 Brownian bridge 随机场，先从 $t=-0.5$ 预演化到 0，再作为正式初值。"
description: "在 Fourier 空间生成 Brownian bridge 随机场，先从 $t=-0.5$ 预演化到 0，再作为正式初值。"
...
| 空间维数 | 2D |
| 时间依赖 | 是 |
| 官方代码标识 | `fluids.incompressible.BrownianBridge` |
| 官方数据页 | [NS-BB](https://huggingface.co/datasets/camlab-ethz/NS-BB) |
...
| 可用物理场 | HF 卡列 $[u_x,u_y,\text{passive tracer}]$；官方论文任务与加载器只用速度。 |
| 轨迹/样本数 | **20000** |
| Train / Val / Test | **19640 / 120 / 240** |
| 官方仓库总文件大小 | **82.6 GB** |
| 网格类型 | 均匀 Cartesian Eulerian 网格 |
| 空间区域 | $D=[0,1]^2$ |
| 初始条件/输入 | 预演化后的 Brownian bridge 速度场。 |
| 边界条件 | 周期边界条件 |
| 保存快照数 | 21 |
...
|---|---|---|---|
| $\alpha_k^{(mn\ell)}$ | Fourier 随机系数；IC / data-distribution parameter | 逐轨迹变化 | $\mathcal U[-1,1]$ |
| $3/2$ | 谱衰减指数；IC / data-distribution parameter | 固定 | $3/2$ |
| pre-evolution | 预演化区间；IC / data-distribution parameter | 固定 | $[-0.5,0]$ |
| $\nu$ | 有效谱黏性；PDE/numerical coefficient | 固定 | $\simeq4\times10^{-4}$ |

**汇总：** 谱/Hurst 指数、预演化时间、黏度和随机系数分布均可调；发布数据只随机化 Fourier 系数。
...
- 原始通道/变量：HF 卡列 $[u_x,u_y,\text{passive tracer}]$；官方论文任务与加载器只用速度。
- 期望组装文件名：`NS-BB.nc`
...
- 低正则、宽频谱随机初值，对长期误差和统计性质更敏感。

## 已知来源差异与复现注意事项

- 官方加载器明确不允许 BrownianBridge tracer 模式，尽管 HF 卡列出第 3 通道。
