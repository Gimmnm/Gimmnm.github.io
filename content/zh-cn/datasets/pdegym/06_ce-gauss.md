---
title: "CE-Gauss：高斯涡量速度初值的可压缩 Euler"
...
summary: "背景密度和压力固定，初始速度由 100 个随机高斯涡量团恢复，再按可压缩 Euler 演化。"
description: "背景密度和压力固定，初始速度由 100 个随机高斯涡量团恢复，再按可压缩 Euler 演化。"

---

# CE-Gauss：高斯涡量速度初值的可压缩 Euler

> **一句话描述：** 背景密度和压力固定，初始速度由 100 个随机高斯涡量团恢复，再按可压缩 Euler 演化。

## 更长描述

该算子把局部涡旋初值与可压缩动力学结合，用于预训练局部涡结构、声学耦合和密度/压力响应。
...
| 空间维数 | 2D |
| 时间依赖 | 是 |
| 官方代码标识 | `fluids.compressible.Gaussians` |
| 官方数据页 | [CE-Gauss](https://huggingface.co/datasets/camlab-ethz/CE-Gauss) |
...
初始速度的涡量使用与 NS-Gauss 相同的 100 个高斯叠加；由不可压关系恢复初始速度。背景初值固定为 $\rho^0=0.1$、$p^0=2.5$。
...
|---|---|---|---|
| $\alpha_i$ | 高斯涡量强度；IC / data-distribution parameter | 逐轨迹变化 | $\mathcal U[-1,1]$ |
| $\sigma_i$ | 高斯宽度；IC / data-distribution parameter | 逐轨迹变化 | $\mathcal U[0.01,0.1]$ |
| $x_i,y_i$ | 中心；IC / data-distribution parameter | 逐轨迹变化 | $\mathcal U[0,1]$ |
| $N_g$ | 高斯个数；IC / data-distribution parameter | 固定 | $100$ |
| $\rho^0$ | 背景密度；IC / data-distribution parameter | 固定 | $0.1$ |
| $p^0$ | 背景压力；IC / data-distribution parameter | 固定 | $2.5$ |
| $\gamma$ | 比热比；PDE coefficient | 固定 | $1.4$ |

**汇总：** 高斯个数、背景密度/压力、$\gamma$、涡量强度与尺度分布均可调；发布数据只改变高斯涡量实现。
