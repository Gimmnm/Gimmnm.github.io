...
1. 原始轨迹通常是 `[N,T,C,H,W]`；官方 all2all 加载器返回 `[C,H,W] → [C,H,W]` 和 lead time，批处理后才有 batch 维。
2. 需区分数学上可调参数、生成代码可改参数与发布数据实际扫描参数。
3. 已知来源冲突包括：Wave-Layer 的 21/15 帧、Wave 数据时间标定、ACE 的 $\epsilon$ 与 $\epsilon^2$ 记法，以及多个 raw tracer 通道未被论文任务使用。
...
- `manifest.csv`：表格索引。
- `TEMPLATE_zh-CN.md`：可复用文档模板。
- `TEMPLATE_en.md`：可复用文档模板。
- `SOURCES.md`：统一来源与版本说明。
