...
CFDBench 不是包含四套完全不同控制方程的 PDE 集合，而是把同一类二维不可压缩 Navier--Stokes 系统放入四种典型 CFD 工况中：顶盖驱动方腔、圆管两相流、带障碍物的重力越障流和圆柱绕流。每个问题配置再分为 BC、PROP、GEO 三个基础子集。

![CFDBench 各问题与子集的 case / 帧数统计](./dataset.png)

![论文常用数学符号定义](./Notations.png)

## 问题配置

| 问题配置 | 物理特征 | 轨迹数 | 总帧数 |
|---|---|---:|---:|
| [Cavity Flow](./cavity_flow/) | 单相、封闭腔、移动壁面 | 159 | 34,582 |
| [Tube Flow](./tube_flow/) | 水--空气两相、入口边界层 | 175 | 39,553 |
| [Dam Flow](./dam_flow/) | 两相、重力、障碍物和射流 | 220 | 21,916 |
| [Cylinder Flow](./cylinder_flow/) | 单相绕流、涡脱落、周期尾迹 | 185 | 205,620 |
| **合计** | — | **739** | **301,671** |
